...
public class OrientationManager implements SensorEventListener {
	...
	private static final float SMOOTH_FACTOR = SMOOTH_THRESHOLD / 18;
	...
	void onSuccess() {
		...
		if (oldOrientation == null) {
			orientation.setX(x);
//			orientation.setY((float)Math.PI - y); //Aplicant aquesta línia, la R.A. estarà invertida.
			orientation.setY(y);
			// lowPassHoritzontalInit(3-y);
			// orientation.setZ(z);
		} else {
			orientation.setX(lowPass(x, oldOrientation.getX()));
//			orientation.setY(lowPass((float) Math.PI - y, oldOrientation.getY()));//Aplicant aquesta línia, la R.A. estarà invertida.
	...
	public interface OnOrientationChangedListener {
		 ...
		 * phone changed
		 * 
		 * @param Orientation on degrees. 360-0 is north.
		 */
