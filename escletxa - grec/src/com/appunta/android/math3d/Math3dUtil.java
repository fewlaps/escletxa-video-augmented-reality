...
public class Math3dUtil {
	...
	public static void getPointPos(Location inLocation, int inPhoneRotation, Vector3 outPos) {
		outPos.z = inLocation.getLatitude();
		outPos.x = inLocation.getLongitude();
		// if (inPhoneRotation == Surface.ROTATION_90 || inPhoneRotation ==
	 ...
	 */
	public static void getRelativeCoordinates(Vector3 inPointPos, Vector3 inCamPos, Trig3 inCamTrig, Vector3 outRelativePos) {
		// Check this article before trying to only understand a simple comma
		...
		outRelativePos.y = inCamTrig.xSin * (inCamTrig.yCos * (inPointPos.z - inCamPos.z) + inCamTrig.ySin * (inCamTrig.zSin * (inPointPos.y - inCamPos.y) + inCamTrig.zCos * (inPointPos.x - inCamPos.x))) + inCamTrig.xCos
				* (inCamTrig.zCos * (inPointPos.y - inPointPos.y) - inCamTrig.zSin * (inPointPos.x - inPointPos.x));
		outRelativePos.z = inCamTrig.xCos * (inCamTrig.yCos * (inPointPos.z - inCamPos.z) + inCamTrig.ySin * (inCamTrig.zSin * (inPointPos.y - inCamPos.y) + inCamTrig.zCos * (inPointPos.x - inCamPos.x))) - inCamTrig.xSin
				* (inCamTrig.zCos * (inPointPos.y - inPointPos.y) - inCamTrig.zSin * (inPointPos.x - inPointPos.x));

	}

	public static void get2dCoordinates(Vector3 inRelativePos, Vector2 inScreenSize, Vector3 inScreenRatio, int inPhoneRotation, Trig1 inScreenRotTrig, Vector2 outScreenPos) {
